defmodule Mmbooking_CloneWeb.UserAuth do
  use Mmbooking_CloneWeb, :verified_routes
  ...
  def log_out_user(conn) do
    ...
    user_token && Authentication.delete_user_session_token(user_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      Mmbooking_CloneWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule Mmbooking_CloneWeb.PageLive do
        use Mmbooking_CloneWeb, :live_view

        on_mount {Mmbooking_CloneWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{Mmbooking_CloneWeb.UserAuth, :ensure_authenticated}] do
  ...
  defp mount_current_user(socket, session) do
    Phoenix.Component.assign_new(socket, :current_user, fn ->
      if user_token = session["user_token"] do
        Authentication.get_user_by_session_token(user_token)
      end
    end)
  end
